...
router.beforeEach((to, from, next) => {
    ...
    if (store.state.selectedSculpture) {
        //fade single sculpture if selected
        let id = store.state.selectedSculpture.id;
        transitionSculptureOpacity(id, 0.0, 1000).then(() => {
            const nextRouteHasSculptureSelected = to.matched.some(
                (record) => record.meta.selectedSculpture
            );
                ...
                setTimeout(() => {
                    ...
                    captureCanvasImage(async (blob) => {
                        try {
                            ...
                            let fileData = await storageRef
                                .child(`sculptureThumbnails/${payload.id}.png`)
                                .put(blob);
...
function init() {
    ...
    mediaCap = piCreateMediaRecorder(
        () => console.log("capturing render"),
        canvas
    );
...
function render(time) {
    ...
    if (
        store.state.canvasSize.width !== prevCanvasSize.width ||
        store.state.canvasSize.height !== prevCanvasSize.height
    ) {
    ...
    if (store.state.selectedSculpture) {
        if (!sculptureHasBeenSelected) {
            ...
            transitionAllSculpturesOpacity(
                0.0,
                1000,
                store.state.selectedSculpture.id
            );
            transitionSculptureOpacity(
                store.state.selectedSculpture.id,
                1.0,
                1000
            );
            ...
            selectedSculpturePose.setFromMatrixPosition(
                store.state.selectedObject.matrixWorld
            );
        ...
        if (!sculptureHasBeenDeselected && store.state.sculpturesLoaded) {
            ...
            transitionAllSculpturesOpacity(
                1.0,
                1000,
                cachedSelectedSculptureId
            );
        } else if (sculptureHasBeenDeselected && cachedCameraPose) {
            tweenObjectToValue(
                camera.position.y,
                store.state.initialCameraPose[1],
                (val) => (camera.position.y = val)
            );
            cachedCameraPose = null;
        }
        sculptureHasBeenSelected = false;
    ...
    store.state.objectsToUpdate.forEach((sculpture) => {
        if (
            !store.state.selectedSculpture &&
            !tweeningSculpturesOpacity &&
            store.state.sculpturesLoaded
        ) {
        ...
        let uniforms = [];
        uniforms.push(
            { name: "time", value: currTime, type: "float" },
            {
                name: "resolution",
                value: new Vector2(
        ...
        if (
            store.state.selectedSculpture &&
            store.state.selectedSculpture.sculpture === sculpture
        ) {
    ...
    if (objectsToRaycast.length > 0) {
        ...
        if (intersects.length > 0) {
            ...
            const frontSideIntersection =
                raycaster.intersectObjects(objectsToRaycast);
            if (frontSideIntersection.length > 0) {
                firstIntersect.material.uniforms.mouse.value =
                    frontSideIntersection[0].point.sub(firstIntersect.position);
            } else {
                firstIntersect.material.uniforms.mouse.value = camera.position
                    .clone()
                    .sub(firstIntersect.position);
            }
            firstIntersect.material.side = BackSide;
            if (
                store.state.selectedSculpture === null &&
                store.state.clickEnabled
            ) {
...
function onMouseMove(event) {
    if (canvasContainer) {
        mouse.x =
            ((event.clientX - canvasContainer.offsetLeft) /
                canvasContainer.clientWidth) *
                2 -
            1;
        mouse.y =
            -(
                (event.clientY - canvasContainer.offsetTop) /
                canvasContainer.clientHeight
            ) *
                2 +
            1;
...
function onMouseUp(event) {
    ...
    if (
        store.state.intersectedObject &&
        store.state.intersectedObject === tempIntersectedObject
    ) {
...
function transitionSculptureOpacity(sculptureId, opacity, duration = 2000) {
    ...
    return new Promise(function (resolve, reject) {
        let sculp = store.state.objectsToUpdate.filter(
            (obj) => obj.mesh.name === sculptureId
        );
        if (sculp.length == 0) {
            reject();
        } else {
...
function transitionAllSculpturesOpacity(
    opacity,
    duration = 2000,
    excludedSculptureId = null
) {
    tweeningSculpturesOpacity = true;

    let objectsToFade = store.state.objectsToUpdate.filter(
        (obj) => calcSculptureOpacityForCameraDistance(obj) > 0
    );
    return new Promise(function (resolve, reject) {
        let fadeSculptures = new TWEEN.Tween(allSculpturesOpacity)
            ...
            .onUpdate(function () {
                objectsToFade.forEach((obj) => {
                    let fadeOpacity =
                        calcSculptureOpacityForCameraDistance(obj);
                    if (!(obj.opacity == 0 && opacity == 0)) {
                        if (!excludedSculptureId && fadeOpacity) {
                            obj.setOpacity(
                                Math.min(
                                    allSculpturesOpacity.opacity,
                                    fadeOpacity
                                )
                            );
