...
export function createPedestalEdges(pedSize, pedHeight, size = 0.005) {
    ...
    const longEdgeGeom = new BoxBufferGeometry(
        edgeSize,
        edgeSize,
        pedSize + 1.0 * edgeSize
    );
    const shortEdgeGeomVertical = new BoxBufferGeometry(
        edgeSize,
        pedHeight,
        edgeSize
    );
    const shortEdgeGeomHorizontal = new BoxBufferGeometry(
        pedSize,
        edgeSize,
        edgeSize
    );
    const edgeMat = new MeshBasicMaterial({ color: new Color(1.0, 1.0, 1.0) });
