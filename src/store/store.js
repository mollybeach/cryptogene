...
export const store = new Vuex.Store({
    ...
    getters: {
        ...
        isAdmin: (state) => {
            return (
                state.user && state.user.uid === "K3lAQQTKbiTiVXlwRZouH4OrWyv1"
            );
        },
    ...
    mutations: {
        ...
        updateSelectedSculpture(state, payload) {
            state.selectedSculpture = Object.assign(
    ...
    actions: {
        ...
        async saveNewSculpture({ commit, getters }, sculpture) {
            ...
            let route =
                sculpture.isExample && getters.isAdmin
                    ? "examples"
                    : "sculptures";
            await firebase
                .database()
        ...
        saveSculpture({ commit, dispatch, getters }, sculptureObj) {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    if (!user) {
                        console.error(
                            "Tried to Save Sculpture when a User is not logged in"
                        );
                        reject(
                            "Tried to Save Sculpture when a User is not logged in"
                        );
                    ...
                    if (!sculpture.id || sculpture.id === sculpture.vueId) {
                        ...
                        if (sculpture.uid === user.uid || getters.isAdmin) {
                            // update existing sculpture
                            let route =
                                sculpture.isExample && getters.isAdmin
                                    ? "examples"
                                    : "sculptures"; //must be admin to update example
                            delete sculpture.vueId; // remove the three.js 3d object

                            resolve(
                                firebase
                                    .database()
                                    .ref(`${route}/${sculpture.id}`)
                                    .update(sculpture)
                                    .then(() => {
                                        commit("setLoading", false);
                                    })
                            ...
                            let route = sculpture.isExample
                                ? "examples"
                                : "sculptures"; //they don't have to be an admin to fork
                            await firebase
                                .database()
                                .ref(`${route}/${sculpture.id}/forks`)
                                .set(newForkCount);
            ...
            if (sculpture.uid === user.uid) {
                ...
                if (sculpture.fork) {
                    firebase
                        .database()
                        .ref("forks")
                        .orderByChild("newSculptureId")
                        .equalTo(sculpture.id)
                        .once("value")
                        .then((data) => {
                            let fork = data.val();
                            if (fork) {
                                ...
                                dispatch("fetchSculpture", {
                                    id: rootSculptureId,
                                    isExample: sculpture.isExample,
                                }).then((sculptureToUpdateForkCount) => {
                                    let route =
                                        sculptureToUpdateForkCount.isExample
                                            ? "examples"
                                            : "sculptures"; //must be admin to update example
                                    ...
                                    firebase
                                        .database()
                                        .ref(
                                            `${route}/${sculptureToUpdateForkCount.id}`
                                        )
                                        .update(sculptureToUpdateForkCount)
                                        .catch((error) => console.error(error));
                                });
                                //delete fork
                                firebase
                ...
                firebase
                    .database()
                    .ref(`users/${user.uid}/sculptures/${sculpture.id}`)
                    .remove()
                    .then(() => {
                        commit("setLoading", false);
                    })
        ...
        fetchUserSculptures({ commit, getters }, username) {
            ...
            return firebase
                ...
                .then((data) => {
                    ...
                    if (sculptures) {
                        return Object.keys(sculptures).map(
                            (key) => sculptures[key]
                        );
                    }
                    return [];
        ...
        fetchSculptures({ commit, getters }, reference) {
            ...
            return firebase
                ...
                .then((data) => {
                    const sculptures = data.val();
                    let output = Object.keys(sculptures).map(
                        (key) => sculptures[key]
                    );
                    // let output = [];
        ...
        addComment({ commit, getters }, { sculpture, comment }) {
            ...
            firebase
                .database()
                .ref(`sculptures/${sculpture.id}/comments`)
                .push({
        ...
        uploadDna({ commit, getters, dispatch }, { dna }) {
            commit("setLoading", true);
            const user = getters.getUser;
            const reference = `users/${user.uid}/dna`;
            return firebase
                .database()
                .ref(reference)
                .set(dna)
                .then(() => {
                    commit("setLoading", false);
                    return dispatch("fetchSculpture", { id: sculpture.id });
                })
                .catch((error) => console.error(error));
        },
        ...
        fetchUserComments({ commit, getters }, uid) {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    let commentsObj = await firebase
                        .database()
                        .ref(`users/${uid}/comments`)
                        .once("value");
        ...
        favorite({ commit, getters }, { sculpture, favorited }) {
            return new Promise(async (resolve, reject) => {
                ...
                if (!user) {
                    console.error(
                        "Tried to favorite Sculpture when a User is not logged in"
                    );
                    reject(
                        "Tried to favorite Sculpture when a User is not logged in"
                    );
                ...
                await firebase
                    .database()
                    .ref(`${route}/${sculpture.id}/favorites`)
                    .transaction((currValue) => {
                        let newFavCount;
                        if (favorited) {
                ...
                if (favorited) {
                    ...
                    if (
                        !(
                            vueXUserFavorites &&
                            vueXUserFavorites.constructor === Object
                        )
                    ) {
                        vueXUserFavorites = {};
                    }
                    ...
                    let favoriteObj = await firebase
                        .database()
                        .ref(userPath)
                        .once("value");
        ...
        fetchUserFavorites({ commit, getters }, uid) {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                    let favoritesObj = await firebase
                        .database()
