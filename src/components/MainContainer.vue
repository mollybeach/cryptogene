<template>
    <div class="main" :class="{embeded: isEmbeded}">
       ...
       <!-- <div style='display:flex' class="actions-bar"></div>
        <actionBar :cachedWidth="actionsBarWidth" :dragging="dragingMouse" :class="{dragging: dragingMouse}" style='display:flex' ></actionBar>-->
    ...
    methods: {
        ...
        mouseMove(event) {
            if(this.dragingMouse) {
                ...
                this.editorWidth = `calc(${newWidth}vw + ${this.handelWidth}px)`;
...
canvas {
    outline: none;
    border: solid 15px var(--spadeblue);
    border-radius: 25px;
    min-height: 500px;
}
